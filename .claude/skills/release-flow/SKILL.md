...
- **Old clients fetch `install.sh` from the release tag, not from main**: a self-updating mole downloads `raw.githubusercontent.com/tw93/mole/V<tag>/install.sh`, and tag content is immutable. An installer/updater bug therefore reaches existing users only through a NEW tag; fixing main alone fixes nobody who is already installed. This is why the V1.47.0 installer fail-open bug forced an immediate V1.47.1 (the V1.47.0 release was pulled the same day).
- **Pulling and re-releasing a version** (done for V1.47.0 -> V1.47.1): `gh release delete V<old> --cleanup-tag` (removes release and remote tag), delete the local tag, close the superseded Homebrew core PR with a one-line supersede comment BEFORE pushing the new tag (an open PR for the same formula can block the workflow's `brew bump-formula-pr`), then bump VERSION + SECURITY_AUDIT, commit `release: V<new>`, tag, and run the normal publish flow. The personal tap and Homebrew core PR regenerate automatically on the new tag; the interim tap state needs no manual repair.
